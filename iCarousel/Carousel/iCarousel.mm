...
NSInteger compareViewDepth(UIView *view1, UIView *view2, iCarousel *self);
...
- (void)setup {
    ...
    [_contentView addGestureRecognizer:leftSwipeGesture];
    ...
    [_contentView addGestureRecognizer:rightSwipeGesture];
...
- (void)setDelegate:(id<iCarouselDelegate>)delegate {
    if (_delegate != delegate) {
        if ((_delegate = delegate) && _dataSource) {
            [self setNeedsLayout];
        }
    }
}
...
- (UIView *)itemViewAtPoint:(CGPoint)point {
    NSEnumerator<UIView*> *depathSortedViews = [[[_itemViews allValues] sortedArrayUsingFunction:(NSInteger (*)(id, id, void *))compareViewDepth context:(__bridge void *)self] reverseObjectEnumerator];
    
    for (UIView *view in depathSortedViews) {
...
- (void)depthSortViews {
    NSArray<UIView*> *depathSortedViews = [[_itemViews allValues] sortedArrayUsingFunction:(NSInteger (*)(id, id, void *))compareViewDepth
...
- (id)viewOrSuperview:(UIView *)view ofClass:(Class)clazz {
    if (!view || view == self.contentView) {
        return nil;
    ...
    return [self viewOrSuperview:view.superview ofClass:clazz];
}

- (void)didSwipe:(UITapGestureRecognizer *)gesture {
    UISwipeGestureRecognizer* swipeGesture = (UISwipeGestureRecognizer*)gesture;
    
    if (swipeGesture.direction == UISwipeGestureRecognizerDirectionLeft) {
        NSInteger count = [self.dataSource numberOfItemsInCarousel:self];
        NSInteger index = (self.currentItemIndex + 1) % count;
    ...
    if (swipeGesture.direction == UISwipeGestureRecognizerDirectionRight) {
        NSInteger count = [self.dataSource numberOfItemsInCarousel:self];
        NSInteger index = (count + self.currentItemIndex - 1) % count;
        [self scrollToItemAtIndex:index animated:YES];
